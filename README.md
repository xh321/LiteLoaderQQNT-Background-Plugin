...
默认加载背景的路径是**插件数据目录（`LiteLoaderQQNT数据目录\background`）**下面的imgs文件夹，在QQ的设置里可以切换背景的目录，保存后立刻生效，目前只会读取目录同级的一些常见格式的图片或视频文件，如下：
...
isCommonBg（对应配置界面`是否对所有窗口共用背景`）：所有窗口的背景是否一致，这在你选择背景图来源为文件夹或者网络API时（并且API返回得是JSON才行）有效，其他情况时此项无效。默认：`true`。
