(() => {
  ...
  const interval3 = setInterval(async () => {
    ...
    if (window.location.href.indexOf("#/main/message") != -1) {
      ...
      // setInterval(() => {
      //   console.log("[Background]", "更新透明度", new Date());
      //   refresh_opacity();
      // }, 100);
      setInterval(async () => {
      ...
      let lastStatus = document.querySelector("body").getAttribute("q-theme");

      setInterval(async () => {
        var currTheme = document.querySelector("body").getAttribute("q-theme");
        if (currTheme != lastStatus) {
          console.log("[Background]", "更新浅色/深色模式", new Date());
          patchCss();
          lastStatus = currTheme;
        }
      }, 1000);
  ...
  //   checkAndSet(
  //     ".container",
  //     "background-image: url('appimg://C:/IDEBackground/18f090040a4172bfdb317fa4f6f69dae.png');background-size: cover;"
  //   );
  // }

  // function checkAndSet(selector, style) {
  //   if (
  ...
  //           .indexOf(style) == -1))
  //   ) {
  //     if (document.querySelector(selector).style == null) {
  ...
  function patchCss() {
    ...
    if (document.querySelector("body").getAttribute("q-theme") == "dark") {
